...
# This script automates installation of autojump for an individual
# user - no root privileges required.

function show_help {
        echo "./install.sh [--prefix ${HOME}/subdir]"
}

# Default install directory: home directory
prefix=$HOME
...
# Assume convention of .bashrc components being placed in
# ${HOME]/.bashrc.d - e.g. see:
# http://www.turnkeylinux.org/blog/generic-shell-hooks - assume no
# further action necessary on this script's part in user's .bashrc.

if [ -d "${HOME}/.bashrc.d" ]; then

    cp autojump.bash ${HOME}/.bashrc.d/
    chmod 755 ${HOME}/.bashrc.d/autojump.bash
